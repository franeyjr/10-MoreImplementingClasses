...
class CapitalT(object):
    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def clone(self):
        ...
        #######################################################################
        intersection_center = rg.Point(self.h_rect.get_center().x, self.h_rect.get_center().y)
        width = self.h_rect.get_upper_right_corner().x - self.h_rect.get_upper_left_corner().x
        height = self.v_rect.get_lower_left_corner().y - self.v_rect.get_upper_left_corner().y
        letter_thickness = self.h_rect.get_lower_left_corner().y - self.h_rect.get_upper_left_corner().y
        clone = CapitalT(intersection_center,width,height,letter_thickness)
